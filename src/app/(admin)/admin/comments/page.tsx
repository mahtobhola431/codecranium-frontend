...
import { MOD_COMMENTS, type CommentStatus } from '@/lib/adminMockData'
...
export default function AdminCommentsPage() {
  const [comments, setComments] = useState(MOD_COMMENTS)
  const [filter, setFilter] = useState<CommentStatus | 'all'>('all')

  const update = (id: string, status: CommentStatus) =>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <span className="text-xs text-zinc-700">›</span>
                  <span className="text-xs text-zinc-600">{comment.lessonTitle}</span>
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {comment.status !== 'approved' && (
                  <button
                    onClick={() => update(comment.id, 'approved')}
                ...
                {comment.status !== 'rejected' && (
                  <button
                    onClick={() => update(comment.id, 'rejected')}
